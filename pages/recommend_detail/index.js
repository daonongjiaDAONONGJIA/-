...
Page({
  ...
  goodsInfo: function () {
    ...
    util.wxRequest(url, params, data => {
      ...
      this.setData({
        ...
        type_date: data.data.type[0].date,
        stock:data.data.type[0].stock
      })
      console.log(data.data.goods_state);
  ...
  changeNum: function (e) {
    ...
    if (cz == 'add') {
      if (this.data.stock > this.data.number){
        number = number + 1;
      }
    } else {
      if (number > 1) {
      ...
      util.wxRequest(url, params, data => {
        if (data.code == 400) {
          ...
          for (var i = 0; i < pages.length; i++) {
            if (pages[i].route == "pages/recommend_detail/index") {
  ...
  selType: function (e) {
    ...
    var type_date = e.currentTarget.dataset.typedate;
